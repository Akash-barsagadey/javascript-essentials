...
const loadLangFromPhrase = async () => {
  ...
  const projectId = 'ff3535bde56f318ee6cbb650b7f75f601';
  const accessToken =
    '35553a6e620c7e1d5d066f68032f357dd28adb7125c6805615b1c65c805d5a8901';
  ...
  console.warn(
    `Phrase app | Pulling data for languages: ${locales.data.map(
      (l) => l.name
    )}`
  );
...
(async () => {
  try {
    ...
    fs.writeFile(
      'src/assets/locales/getLangData.json',
      JSON.stringify(data),
      function (err) {
        if (err) {
          throw err;
